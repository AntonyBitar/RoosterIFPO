...
// Package to make DropDown body(after open) greater than DropDownField
...
typedef FilterCallback<T> = List<DropdownMenusCodeCodeEntry<T>> Function(List<DropdownMenusCodeCodeEntry<T>> entries, String filter);
...
typedef SearchCallback<T> = int? Function(List<DropdownMenusCodeCodeEntry<T>> entries, String query);
...
class DropdownMenusCodeCodeEntry<T> {
  ...
  ///
  /// The default value is true. If true, the [DropdownMenusCodeCodeEntry.label] will be filled
...
///
/// The menu is composed of a list of [DropdownMenusCodeCodeEntry]s. People can provide information,
...
/// * [DropdownMenusCodeCodeEntry], which is used to build the [MenuItemButton] in the [DropdownMenu] list.
class DropdownMenusCodeCode<T> extends StatefulWidget {
  ...
  /// Descriptions of the menu items in the [DropdownMenusCodeCode].
  ///
  /// This is a required parameter. It is recommended that at least one [DropdownMenusCodeCodeEntry]
  /// is provided. If this is an empty list, the menu will be empty and only
  /// contain space for padding.
  final List<DropdownMenusCodeCodeEntry<T>> dropdownMenusEntries;
  ...
  /// by the width of menu items or [DropdownMenusCodeCode.width]. If this property is not null,
  ...
  /// DropdownMenu<Text>(
  ///   enableFilter: true,
  ///   filterCallback: (List<DropdownMenusCodeCodeEntry<Text>> entries, String filter) {
  ...
  ///
  ///       return entries
  ///         .where((DropdownMenusCodeCodeEntry<Text> entry) =>
  ///           entry.label.toLowerCase().contains(trimmedFilter),
  ///         )
  ///         .toList();
  ///   },
  ///   dropdownMenuEntries: const <DropdownMenusCodeCodeEntry<Text>>[],
  /// )
  /// ```
  /// {@end-tool}
  ///
  ...
  /// ```dart
  /// DropdownMenu<Text>(
  ///   searchCallback: (List<DropdownMenusCodeCodeEntry<Text>> entries, String query) {
  ///     if (query.isEmpty) {
  ///       return null;
  ///     }
  ///     final int index = entries.indexWhere((DropdownMenusCodeCodeEntry<Text> entry) => entry.label == query);
  ///
  ...
  /// {@end-tool}
  ///
  /// Defaults to null. If this is null and [DropdownMenusCodeCode.enableSearch] is true,
  ...
  @override
  State<DropdownMenusCodeCode<T>> createState() => _DropdownMenusCodeCodeState<T>();
}

class _DropdownMenusCodeCodeState<T> extends State<DropdownMenusCodeCode<T>> {
  ...
  late List<DropdownMenusCodeCodeEntry<T>> filteredEntries;
  ...
  void initState() {
    ...
    _menuHasEnabledItem = filteredEntries.any((DropdownMenusCodeCodeEntry<T> entry) => entry.enabled);

    final int index = filteredEntries.indexWhere((DropdownMenusCodeCodeEntry<T> entry) => entry.value == widget.initialSelection);
    if (index != -1) {
  ...
  @override
  void didUpdateWidget(DropdownMenusCodeCode<T> oldWidget) {
  ...
  List<DropdownMenusCodeCodeEntry<T>> filter(List<DropdownMenusCodeCodeEntry<T>> entries, TextEditingController textEditingController) {
    final String filterText = textEditingController.text.toLowerCase();
    return entries
        .where((DropdownMenusCodeCodeEntry<T> entry) => entry.label.toLowerCase().contains(filterText))
        .toList();
  }

  bool _shouldUpdateCurrentHighlight(List<DropdownMenusCodeCodeEntry<T>> entries) {
  ...
  int? search(List<DropdownMenusCodeCodeEntry<T>> entries, TextEditingController textEditingController) {
    final String searchText = textEditingController.value.text.toLowerCase();
    if (searchText.isEmpty) {
      return null;
    }

    final int index = entries.indexWhere((DropdownMenusCodeCodeEntry<T> entry) => entry.label.toLowerCase().contains(searchText));

    return index != -1 ? index : null;
  }

  List<Widget> _buildButtons(
      List<DropdownMenusCodeCodeEntry<T>> filteredEntries,
    ...
    for (int i = 0; i < filteredEntries.length; i++) {
      final DropdownMenusCodeCodeEntry<T> entry = filteredEntries[i];
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.width != null) {
      effectiveMenuStyle = effectiveMenuStyle.copyWith(minimumSize: WidgetStatePropertyAll<Size?>(Size(430, 0.0)));
    ...
    Widget menuAnchor = MenuAnchor(
      ...
      builder: (BuildContext context, MenuController controller, Widget? child) {
        ...
        final Widget textField = TextField(
            ...
            onEditingComplete: () {
              if (currentHighlight != null) {
                final DropdownMenusCodeCodeEntry<T> entry = filteredEntries[currentHighlight!];
                if (entry.enabled) {
                  _localTextEditingController?.value = TextEditingValue(
                    text: entry.label,
                    selection: TextSelection.collapsed(offset: entry.label.length),
